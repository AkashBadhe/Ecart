...
await $`pnpm --dir /var/www/pickbazar-react install`;
...
if (whichConfig == 1) {
    ...
    await $`pm2 --name shop-rest start pnpm -- --dir /var/www/pickbazar-react start:shop-rest`;

    echo(chalk.blue("Running For Admin Rest API with pm2"));

    await $`pm2 --name admin-rest start pnpm -- --dir /var/www/pickbazar-react start:admin-rest`;
} else {
    echo(chalk.blue("Running For Shop GraphQL API with pm2"));

    await $`pm2 --name shop-gql start pnpm -- --dir /var/www/pickbazar-react start:shop-gql`;

    echo(chalk.blue("Running For Admin GraphQL API with pm2"));

    await $`pm2 --name admin-gql start pnpm -- --dir /var/www/pickbazar-react start:admin-gql`;
}
